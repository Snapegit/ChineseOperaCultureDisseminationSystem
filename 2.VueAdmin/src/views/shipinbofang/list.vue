	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.xiqumingcheng" placeholder="戏曲名称"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.xiquliupai" placeholder="戏曲流派"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('shipinbofang','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('shipinbofang','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('shipinbofang','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shipinbofang','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('shipinbofang','视频类型统计')">视频类型统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('shipinbofang','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('shipinbofang','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'视频类型统计'" width="70%">
			<div  id="xiqufenleiEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.xiqumingcheng&&searchQuery.value.xiqumingcheng!=''){
			params['xiqumingcheng'] = '%' + searchQuery.value.xiqumingcheng + '%'
		}
		if(searchQuery.value.xiquliupai&&searchQuery.value.xiquliupai!=''){
			params['xiquliupai'] = '%' + searchQuery.value.xiquliupai + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var xiqufenleiEchart1 = echarts.init(document.getElementById("xiqufenleiEchart1"),'macarons');
			context?.$http({
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussshipinbofang?refid=' + id)
	}
	//初始化
	const init = () => {
		getList()
